...
namespace Test
{
    ...
    class moonscr
    {
            ...
            if (tocee)
            {
                if (Mods.ifExists(Mods.getFiles(Mods.getCrntDir()), ".lua"))
                {
                    foreach (string nfs in Mods.getFiles(Mods.getCrntDir()))
                    {
                        if (nfs.EndsWith(".lua"))
                        {
                            moonscr.luaaStatic(nfs, Mods.getFiles(Mods.getCrntDir()), "/home/lynxishdevelop/ll_53/liblua.a", "-I/home/lynxishdevelop/ll_53");
                        }
                    }
                }
            }
        ...
        public static void luaaStatic(string filoo, string[] ppfils, string wliblua, string wluadir)
        {
            ...
            Mods.dueProcess("luastatic", filoo + " " + libdirr + " " + luadirr);
        }
